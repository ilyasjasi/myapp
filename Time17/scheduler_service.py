...
class SchedulerService:
    ...
    def setup_flask_context(self):
        ...
        try:
            ...
            # Use direct SQLite connection for scheduler operations
            self.db_path = "instance/attendance.db"
            
            # Test database connection
            conn = sqlite3.connect('instance/attendance.db')
            conn.close()
            
            logging.info("Database connection setup completed")
            return True
    ...
    def device_sync_job(self):
        """Sync devices with limited scope to prevent crashes"""
        import sqlite3
        import json
        ...
        cursor.execute("""
            INSERT INTO job_executions (job_id, job_name, status, start_time)
            VALUES (?, ?, ?, ?)
        """, ('auto_device_sync', 'Auto Device Sync', 'running', datetime.utcnow().isoformat()))
        execution_id = cursor.lastrowid
        conn.commit()
        ...
        try:
            ...
            # Only sync 3 devices at a time to prevent overload
            # Use round-robin rotation based on last sync time
            cursor.execute("""
            ...
            for device in devices:
                try:
                    ...
                    # Quick sync with timeout
                    start_time = time.time()
                    timeout = 20  # 20 seconds max per device
                    
                    if time.time() - start_time < timeout:
                        # Sync users from device
                        user_count = device_mgr.sync_users_from_device(
                            ip_address, 
                            device_id, 
                            area_id
                        )
                        ...
                        # No need to duplicate log sync here
                        
                        if user_count > 0:
                            synced_count += 1
                            logging.info(f"Synced {user_count} users from {ip_address}")
                    ...
                    # Quick disconnect
                    device_mgr.disconnect_device(ip_address)
