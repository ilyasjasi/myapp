...
try:
    from enhanced_device_sync import sync_devices_in_area as enhanced_sync_devices_in_area
    ENHANCED_SYNC_AVAILABLE = True
except ImportError:
    ENHANCED_SYNC_AVAILABLE = False
    logging.warning("Enhanced sync module not available.")
...
class DeviceManager:
    ...
    def disconnect_device(self, ip_address):
        """Disconnect from a device and clean up connection cache"""
        if ip_address in self.connections:
            conn, _ = self.connections[ip_address]
            try:
                conn.disconnect()
            except:
                pass
            del self.connections[ip_address]
            logging.debug(f"Disconnected from device {ip_address}")
    ...
    def sync_devices_in_area(self, area_id):
        """Enhanced comprehensive sync of all devices in an area - users, fingerprint and face templates"""
        try:
            if ENHANCED_SYNC_AVAILABLE:
                return enhanced_sync_devices_in_area(area_id)
            else:
                logging.error("Enhanced sync module not available")
                return {'success': False, 'synced_devices': 0, 'total_users_synced': 0, 'total_templates_synced': 0, 'error': 'Module not available'}
        except Exception as e:
            logging.error(f"Error in enhanced area sync: {e}")
            return {'success': False, 'synced_devices': 0, 'total_users_synced': 0, 'total_templates_synced': 0, 'error': str(e)}
            
            logging.info(f"Starting comprehensive sync for {len(devices)} devices in area {area_id}")
            
            # Connect to all devices and collect data
            device_connections = {}
            device_data = {}
            ...
            for device_id, ip_address in devices:
                try:
                    ...
                    if conn:
                        ...
                        # Get templates for each user (fingerprint and face)
                        for user in users:
                            try:
                                # Get fingerprint templates
                                finger_templates = conn.get_user_template(uid=user.uid)
                                if finger_templates:
                                    device_data[ip_address]['templates'][user.user_id] = {
                                        'fingerprint': finger_templates,
                                        'face': None
                                    }
                                
                                # Try to get face templates if supported
                                try:
                                    if hasattr(conn, 'get_face_template'):
                                        face_template = conn.get_face_template(uid=user.uid)
                                        if face_template:
                                            if user.user_id not in device_data[ip_address]['templates']:
                                                device_data[ip_address]['templates'][user.user_id] = {
                                                    'fingerprint': None,
                                                    'face': face_template
                                                }
                                            else:
                                                device_data[ip_address]['templates'][user.user_id]['face'] = face_template
                                except Exception as face_e:
                                    logging.debug(f"No face template for user {user.user_id} on {ip_address}: {face_e}")
                                    
                            except Exception as e:
                                logging.debug(f"No templates for user {user.user_id} on {ip_address}: {e}")
                        
                        logging.info(f"Device {ip_address}: {len(users)} users, {len(device_data[ip_address]['templates'])} templates")
                        
                except Exception as e:
                    logging.error(f"Failed to connect to device {ip_address}: {e}")
            
            if len(device_connections) < 2:
                logging.error("Could not connect to enough devices for sync")
                return {'synced_devices': 0, 'total_users_synced': 0, 'total_templates_synced': 0}
            
            # Find primary device (most users + templates)
            primary_ip = max(device_data.keys(), 
                           key=lambda ip: device_data[ip]['user_count'] + len(device_data[ip]['templates']))
            
            logging.info(f"Primary device selected: {primary_ip} with {device_data[primary_ip]['user_count']} users")
            ...
            primary_users = device_data[primary_ip]['users']
            primary_templates = device_data[primary_ip]['templates']
            
            for target_ip in device_data.keys():
                if target_ip == primary_ip:
                    continue
                
                try:
                    target_conn = device_connections[target_ip]
                    target_users = device_data[target_ip]['users']
                    target_templates = device_data[target_ip]['templates']
                    ...
                    for user_id, user in primary_users.items():
                        if user_id not in target_users:
                            try:
                                ...
                                users_added += 1
                                logging.info(f"Added user {user_id} to {target_ip} with UID {max_uid}")
                                
                                # Add templates for this user
                                if user_id in primary_templates:
                                    template_data = primary_templates[user_id]
                                    try:
                                        # Add fingerprint templates
                                        if isinstance(template_data, dict) and template_data.get('fingerprint'):
                                            target_conn.save_user_template(
                                                user=max_uid,
                                                fingers=template_data['fingerprint']
                                            )
                                            templates_added += 1
                                            logging.info(f"Added fingerprint templates for user {user_id} to {target_ip}")
                                        elif not isinstance(template_data, dict):
                                            # Legacy format - assume fingerprint
                                            target_conn.save_user_template(
                                                user=max_uid,
                                                fingers=template_data
                                            )
                                            templates_added += 1
                                            logging.info(f"Added fingerprint templates for user {user_id} to {target_ip}")
                                        
                                        # Add face templates if supported and available
                                        if isinstance(template_data, dict) and template_data.get('face'):
                                            try:
                                                if hasattr(target_conn, 'save_face_template'):
                                                    target_conn.save_face_template(uid=max_uid, template=template_data['face'])
                                                    logging.info(f"Added face template for user {user_id} to {target_ip}")
                                            except Exception as face_e:
                                                logging.warning(f"Failed to add face template for user {user_id}: {face_e}")
                        ...
                        if user_id in target_users and user_id not in target_templates:
                            try:
                                ...
                                if isinstance(template_data, dict) and template_data.get('fingerprint'):
                                    ...
                                    templates_added += 1
                                    logging.info(f"Added fingerprint templates for existing user {user_id} on {target_ip}")
                                elif not isinstance(template_data, dict):
                                    # Legacy format - assume fingerprint
                                    target_conn.save_user_template(
                                        user=target_user_uid,
                                ...
                                if isinstance(template_data, dict) and template_data.get('face'):
                                    try:
                                        if hasattr(target_conn, 'save_face_template'):
                                            target_conn.save_face_template(uid=target_user_uid, template=template_data['face'])
                                            logging.info(f"Added face template for existing user {user_id} on {target_ip}")
                                    except Exception as face_e:
                                        logging.warning(f"Failed to add face template for existing user {user_id}: {face_e}")
                                        
                            except Exception as e:
                                logging.warning(f"Failed to add templates for existing user {user_id}: {e}")
                    
                    # Now sync back from target to primary (bidirectional sync)
                    for user_id, user in target_users.items():
                        if user_id not in primary_users:
                            try:
                                primary_conn = device_connections[primary_ip]
                                primary_existing_uids = {u.uid for u in primary_conn.get_users()}
                                primary_max_uid = max(primary_existing_uids) if primary_existing_uids else 0
                                primary_max_uid += 1
                                ...
                                users_added += 1
                                logging.info(f"Added user {user_id} to primary {primary_ip}")
                                
                                # Add templates
                                if user_id in target_templates:
                                    template_data = target_templates[user_id]
                                    try:
                                        # Add fingerprint templates
                                        if isinstance(template_data, dict) and template_data.get('fingerprint'):
                                            primary_conn.save_user_template(
                                                user=primary_max_uid,
                                                fingers=template_data['fingerprint']
                                            )
                                            templates_added += 1
                                            logging.info(f"Added fingerprint templates for user {user_id} to primary {primary_ip}")
                                        elif not isinstance(template_data, dict):
                                            # Legacy format - assume fingerprint
                                            primary_conn.save_user_template(
                                                user=primary_max_uid,
                                        ...
                                        if isinstance(template_data, dict) and template_data.get('face'):
                                            try:
                                                if hasattr(primary_conn, 'save_face_template'):
                                                    primary_conn.save_face_template(uid=primary_max_uid, template=template_data['face'])
                                                    logging.info(f"Added face template for user {user_id} to primary {primary_ip}")
                                            except Exception as face_e:
                                                logging.warning(f"Failed to add face template for user {user_id} to primary: {face_e}")
                                                
                                    except Exception as e:
                                        logging.warning(f"Failed to add templates for user {user_id} to primary: {e}")
                                        
                            except Exception as e:
                                logging.error(f"Failed to add user {user_id} to primary {primary_ip}: {e}")
                    
                    total_users_synced += users_added
                    total_templates_synced += templates_added
                    synced_devices += 1
                    
                    logging.info(f"Synced to {target_ip}: {users_added} users, {templates_added} templates")
            ...
            logging.info(f"Area {area_id} sync completed: {result}")
            return result
            
        except Exception as e:
            logging.error(f"Error in comprehensive area sync: {e}")
            return {'synced_devices': 0, 'total_users_synced': 0, 'total_templates_synced': 0, 'error': str(e)}
            ...
            try:
                ...
                for template in source_templates:
                    if (template.uid, template.fid) not in target_template_uids:
                        try:
                            # Template sync is complex and requires proper user context
                            # For now, skip template sync and focus on user sync
                            logging.debug(f"Skipping template sync for UID:{template.uid} FID:{template.fid} (requires user context)")
                            continue
                        except Exception as e:
    ...
    def push_users_to_device(self, target_ip, area_id):
        ...
        try:
            ...
            db_path = os.path.join(project_dir, 'instance', 'attendance.db')
            
            # Get all active users in the same area using direct SQLite query
            ...
            for user_data in users:
                try:
                    ...
                    conn.set_user(
                        uid=uid,
                        name=f"{first_name or ''} {last_name or ''}".strip() or f"User{user_id}",
    ...
    def remove_terminated_users_from_device(self, ip_address):
        ...
        try:
            ...
            # Get terminated users from database using direct SQLite query
            import sqlite3
            import os
            ...
            conn_db = sqlite3.connect(db_path)
            cursor = conn_db.cursor()
            cursor.execute("SELECT user_id FROM users WHERE status = 'Terminated'")
            terminated_users = cursor.fetchall()
            conn_db.close()
            
            terminated_user_ids = {str(user[0]) for user in terminated_users}
    ...
    def comprehensive_device_sync(self, ip_address, area_id):
        """Comprehensive device sync: sync users and templates TO device, remove terminated users, sync time"""
        try:
            import sqlite3
            import os
            
            logging.info(f"Starting comprehensive sync for device {ip_address}")
            ...
            conn_db = sqlite3.connect(db_path)
            cursor = conn_db.cursor()
            cursor.execute("SELECT name, area_id FROM devices WHERE ip_address = ?", (ip_address,))
            device_info = cursor.fetchone()
            if not device_info:
                logging.error(f"Device {ip_address} not found in database")
                conn_db.close()
                return {'success': False, 'error': 'Device not found'}
            
            device_name, device_area_id = device_info
            actual_area_id = area_id or device_area_id
            conn_db.close()
            ...
            try:
                ...
                try:
                    ...
                    if hasattr(conn, 'set_time'):
                        conn.set_time(datetime.now())
                        results['time_synced'] = True
                        logging.info(f"Synced time to device {ip_address}")
                except Exception as e:
                    results['errors'].append(f"Time sync failed: {str(e)}")
                    logging.warning(f"Failed to sync time to device {ip_address}: {e}")
                
                # 2. Remove terminated users first
                try:
                    terminated_count = self.remove_terminated_users_from_device(ip_address)
                    results['terminated_removed'] = terminated_count
                    logging.info(f"Removed {terminated_count} terminated users from device {ip_address}")
                except Exception as e:
                    results['errors'].append(f"Terminated user removal failed: {str(e)}")
                    logging.error(f"Error removing terminated users from device {ip_address}: {e}")
                
                # 3. Bidirectional user sync - first collect FROM device TO database, then push TO device
                try:
                    # Step 3a: Sync users FROM device TO database (collect new users)
                    users_collected = self.sync_users_from_device(ip_address, device_name, actual_area_id)
                    results['users_collected'] = users_collected
                    logging.info(f"Collected {users_collected} users from device {ip_address} to database")
                    
                    # Step 3b: Sync active users TO device based on area
                    users_added = 0
                    if actual_area_id:
                        # Get active users for this area using direct SQLite query
                        conn_db = sqlite3.connect(db_path)
                        cursor = conn_db.cursor()
                        cursor.execute("""
                            SELECT user_id, first_name, last_name 
                        ...
                        device_users = conn.get_users() or []
                        existing_user_ids = {str(getattr(u, 'user_id', '')) for u in device_users}
                        
                        # Get next available UID
                        next_uid = max([getattr(u, 'uid', 0) for u in device_users], default=0) + 1
                        ...
                        for user_data in active_users:
                            try:
                                ...
                                if str(user_id) not in existing_user_ids:
                                    ...
                                    users_added += 1
                                    next_uid += 1
                                    logging.info(f"Added user {user_id} to device {ip_address}")
                                ...
                                results['errors'].append(f"Failed to add user {user.user_id}: {str(e)}")
                                logging.warning(f"Failed to add user {user.user_id} to device {ip_address}: {e}")
                                continue
                    
                    results['users_synced'] = users_added
                    logging.info(f"Synced {users_added} users to device {ip_address}")
                    
                except Exception as e:
                    results['errors'].append(f"User sync failed: {str(e)}")
                    logging.error(f"Error syncing users for device {ip_address}: {e}")
                
                # 4. Sync biometric templates device-to-device (not from/to database)
                try:
                    if actual_area_id:
                        # Templates are stored only on devices, sync between devices in same area
                        templates_synced = self.sync_templates_in_area(actual_area_id, ip_address)
                        results['templates_synced'] = templates_synced
                        logging.info(f"Synced {templates_synced} templates between devices for {ip_address}")
                    
                except Exception as e:
                    results['errors'].append(f"Template sync failed: {str(e)}")
                    logging.error(f"Error syncing templates for device {ip_address}: {e}")
                
                # 5. Collect logs (optional - this is usually done by separate job)
                try:
                    logs_collected = self.collect_logs_from_device(ip_address)
                    results['logs_collected'] = logs_collected
                    logging.info(f"Collected {logs_collected} logs from device {ip_address}")
                except Exception as e:
                    results['errors'].append(f"Log collection failed: {str(e)}")
                    logging.warning(f"Error collecting logs from device {ip_address}: {e}")
            ...
            logging.info(f"Comprehensive sync completed for device {ip_address}: {results}")
            return results
            
        except Exception as e:
            logging.error(f"Error in comprehensive device sync for {ip_address}: {e}")
            return {'success': False, 'error': str(e)}

    def sync_templates_in_area(self, area_id, target_device_ip):
        """Sync templates and users comprehensively across all devices in the area"""
        try:
            # Use the comprehensive area sync method
            sync_result = self.sync_devices_in_area(area_id)
            
            # Return the number of templates synced
            return sync_result.get('total_templates_synced', 0)
            
        except Exception as e:
            logging.error(f"Error syncing templates in area {area_id}: {e}")
            return 0